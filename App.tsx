...
const App: React.FC = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Sometimes the API returns a string that needs parsing, even with schema
      if (typeof result === 'string') {
         try {
            const parsedResult = JSON.parse(result);
            setSolutions(parsedResult);
         } catch (e) {
            console.error("Failed to parse string result from API:", e);
            setError("AI 回傳的格式有誤，請稍後再試。");
         }
      ...
      console.error(err);
      setError('解題時發生錯誤，請檢查您的網路連線或稍後再試。');
    } finally {
      setIsLoading(false);
    }
  }, [imageFile]);
